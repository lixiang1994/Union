...
-(void)setTime:(CGFloat)time {
    _time = time;
    if (time <= 0.01) {
        self.type = LXScrollLabelTypeBan;
    }
    else {
...
-(void)setType:(LXScrollLabelType)type {
    _type = type;
    switch (type) {
        case LXScrollLabelTypeRepeat:
            [self startAnimation];
            break;
            
        case LXScrollLabelTypeClick:
            [self stopAnimation];
            break;
            
        case LXScrollLabelTypeBan:
            [self stopAnimation];
            break;
            
        default:
            _type = LXScrollLabelTypeRepeat;
            [self startAnimation];
            break;
    }
}
...
-(void)startAnimation {
    if (self.time < 0.1     // 时间过小
        || self.type == LXScrollLabelTypeBan // 不滚动类型
...
-(void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    if (flag) {
        _isRight = !_isRight;
        if (self.type == LXScrollLabelTypeRepeat) {
            [self startAnimation];
        }
        else if (self.type == LXScrollLabelTypeClick) {
            if (_isRight) {
                [self startAnimation];
            }
            else {
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    if (self.type == LXScrollLabelTypeRepeat
        || self.type == LXScrollLabelTypeClick) {
 ...
 */
@implementation LXScrollLabel (UILabelProperty)
